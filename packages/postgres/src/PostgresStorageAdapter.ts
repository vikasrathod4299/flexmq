...
import { Client, Pool, type Notification, type PoolConfig } from "pg";
...
export class PostgresStorageAdapter<T> implements StorageAdapter<T> {
  ...
  private capacityWaiters: Map<string, Set<() => void>> = new Map();
  private terminalWaiters: Map<string, Set<() => void>> = new Map();
  ...
  constructor(config: PostgresConfig) {
    ...
    this.notificationClient = new Client(poolConfig);
  }
  ...
  async connect(): Promise<void> {
    ...
    this.notificationClient.on("notification", this.handleNotification);
    await this.notificationClient.query("LISTEN flexmq_events");
  ...
  async disconnect(): Promise<void> {
    ...
    this.notificationClient.off("notification", this.handleNotification);

    this.flushWaiters(this.capacityWaiters);
    this.flushWaiters(this.terminalWaiters);

    try {
      await this.notificationClient.query("UNLISTEN flexmq_events");
    } finally {
  ...
  async ensureSchema(): Promise<void> {
    ...
    await this.pool.query(`
      CREATE INDEX IF NOT EXISTS flexmq_jobs_pending_idx
      ON flexmq_jobs (queue_name, status, created_at)
    `);
    await this.pool.query(`
      CREATE INDEX IF NOT EXISTS flexmq_jobs_delayed_idx
      ON flexmq_jobs (queue_name, status, next_attempt_at)
    `);
    await this.pool.query(`
      CREATE INDEX IF NOT EXISTS flexmq_jobs_processing_idx
      ON flexmq_jobs (queue_name, status, lease_until)
    `);
    await this.pool.query(`
      CREATE INDEX IF NOT EXISTS flexmq_job_events_queue_idx
      ON flexmq_job_events (queue_name, event_id)
    `);
    await this.pool.query(`
      CREATE INDEX IF NOT EXISTS flexmq_job_events_job_idx
      ON flexmq_job_events (queue_name, job_id, event_id)
    `);
  }
  ...
  async enqueue(queueName: string, job: Job<T>): Promise<boolean> {
    ...
    try {
      ...
      await client.query(`SELECT pg_advisory_xact_lock(hashtext($1))`, [queueName]);
      ...
      await client.query(
        `
          ...
          VALUES ($1, $2, 'job_available', 'pending', NULL)
        `,
        [queueName, job.id]
      );

      await client.query(`SELECT pg_notify('flexmq_events', $1)`, [queueName]);
  ...
  async claim(queueName: string, options: ClaimOptions): Promise<Claim<T> | null> {
    ...
    do {
      ...
      await this.waitForWakeHint(this.capacityWaiters, queueName, remainingMs);
  ...
  async promoteDelayedJobs(queueName: string, now?: number): Promise<number> {
    ...
    const result = await this.pool.query(
      ...
      `,
      [queueName, effectiveNow]
    );

    if ((result.rowCount ?? 0) > 0) {
      await this.pool.query(`SELECT pg_notify('flexmq_events', $1)`, [queueName]);
    }
  ...
  async waitForTerminalState(
    ...
    do {
      ...
      await this.waitForWakeHint(this.terminalWaiters, queueName, remainingMs);
    } while (Date.now() < deadline);

    const finalJob = await this.getJob(queueName, jobId);
    return finalJob && this.isTerminalJob(finalJob) ? finalJob : null;
  }
  ...
  private async waitForWakeHint(
    ...
    await new Promise<void>((resolve) => {
      ...
      const queueWaiters = waiters.get(queueName) ?? new Set<() => void>();
      queueWaiters.add(wake);
      waiters.set(queueName, queueWaiters);

      const cleanup = () => {
        clearTimeout(timer);
        const activeWaiters = waiters.get(queueName);
        if (!activeWaiters) {
          return;
        }
  ...
  private handleNotification = (msg: Notification): void => {
    if (msg.channel !== "flexmq_events" || !msg.payload) {
      return;
    }

    this.resolveWaiters(this.capacityWaiters, msg.payload);
    this.resolveWaiters(this.terminalWaiters, msg.payload);
  };

  private resolveWaiters(waiters: Map<string, Set<() => void>>, queueName: string): void {
    const queueWaiters = waiters.get(queueName);
    if (!queueWaiters || queueWaiters.size === 0) {
      return;
    }

    waiters.delete(queueName);
    for (const wake of queueWaiters) {
      wake();
    }
  }

  private flushWaiters(waiters: Map<string, Set<() => void>>): void {
  ...
  private mapRowToJob(row: JobRow<T>): Job<T> {
    return {
      ...
      processingStartedAt: row.processing_started_at
        ? row.processing_started_at.getTime()
        : undefined,
    };
  }
}
